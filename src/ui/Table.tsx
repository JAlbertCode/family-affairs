...
import { CharacterCard, EmptySlot, slotName, charName } from './CharacterCard'
        ...
        <div className="hand">
          {me.hand.map((iid) => {
            ...
            return (
              <button
                key={iid}
                className={`hcard ${selected ? 'sel' : ''}`}
                style={{ borderColor: (chDef?.color ?? stDef?.color ?? '#43284a') + '99', opacity: playable ? 1 : .45 }}
                onClick={() => tapHandCard(iid)}
                disabled={!playable}
              >
                <span className="htype" style={{ color: chDef?.color ?? stDef?.color }}>
                  {chDef ? 'CHARACTER' : stDef?.subtype}
                </span>
                <span className="hn">{chDef?.name ?? stDef?.name}</span>
                <span className="ht">
                  {chDef
                    ? `${chDef.stats.hp} HP · ⚔${chDef.stats.attack} 🛡${chDef.stats.defense} ⚡${chDef.stats.speed}`
                    : stDef?.text}
                </span>
      ...
      <div className="sheet battle">
        ...
        <div className="rolls">
          <div>
            ...
            <div className="hp-text">{charName(state, b.attackerChar)}</div>
          </div>
          <span className="vs">VS</span>
          <div>
            <div className="die">🛡</div>
            <div className="hp-text">{charName(state, b.defenderChar)}</div>
          </div>
        </div>
        <div className="sub">
